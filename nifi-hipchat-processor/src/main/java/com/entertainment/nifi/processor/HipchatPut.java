...
/* adpated from https://github.com/seanbyrne88/nifi_templates */
...
public class HipchatPut extends AbstractProcessor {

    public static final String HIPCHAT_BASEURL_TEMPLATE_V2 =
            "https://api.hipchat.com/v2/room/{room_id}/notification?auth_token={auth_token}";

    public static final String HIPCHAT_BASEURL_TEMPLATE_V1 =
            "https://api.hipchat.com/v1/rooms/message?auth_token={auth_token}"; //room_id=ourRoom&from=BuildBot&message=Build+Status:+Failing&color=red&notify=1
    ...
    public static final PropertyDescriptor API_VERSION = new PropertyDescriptor
            ...
            .description("The Hipchat API version")
            .required(true)
            .expressionLanguageSupported(true)
            .addValidator(StandardValidators.NON_EMPTY_VALIDATOR)
            .defaultValue("v1")
    ...
    public static final PropertyDescriptor FROM = new PropertyDescriptor
            ...
            .name("username")
            .displayName("From")
            .description("The auth key will determine who the notification will show from in v2, but this parameter will let you add username for v1 api")
            .required(false)
            .expressionLanguageSupported(true)
            .addValidator(StandardValidators.NON_EMPTY_VALIDATOR)
            .defaultValue("Apache Nifi")
            .build();
    ...
    public static final List<PropertyDescriptor> descriptors = Collections.unmodifiableList(
            Arrays.asList(ROOM_ID, MESSAGE_TEXT, FROM, MESSAGE_BG_COLOR, AUTH_TOKEN, API_VERSION));
    ...
    private  static String buildHipchatV1Url(String base, String authToken){
        return base.replace("{auth_token}", authToken);
    }
    ...
    public void onTrigger(final ProcessContext context, final ProcessSession session) {
        ...
        String api_version = context.getProperty(HipchatPut.API_VERSION).evaluateAttributeExpressions(flowFile).getValue();
        if("v1".equalsIgnoreCase(api_version)) {
            sendV1Notification(context,session,flowFile);
        }else {
            sendV2Notification(context, session, flowFile);
        }
    }

    private void sendV2Notification(ProcessContext context, ProcessSession session, FlowFile flowFile) {
        ...
        String hipchat_url = buildHipchatUrl(HIPCHAT_BASEURL_TEMPLATE_V2, roomID, authToken);
    ...
    private void sendV1Notification(ProcessContext context, ProcessSession session, FlowFile flowFile) {
        String roomID = context.getProperty(ROOM_ID).evaluateAttributeExpressions().getValue();
        String message = context.getProperty(MESSAGE_TEXT).evaluateAttributeExpressions(flowFile).getValue();
        String from = context.getProperty(FROM).evaluateAttributeExpressions(flowFile).getValue();
        String color = context.getProperty(MESSAGE_BG_COLOR).getValue();

        //to support room name we need to encode roomID for any spaces or other special characters used in Hipchat room name

        String authToken = context.getProperty(AUTH_TOKEN).getValue();
        String hipchat_url = buildHipchatV1Url(HIPCHAT_BASEURL_TEMPLATE_V1, authToken);
        getLogger().debug("Hipchat v1 URL: "+ hipchat_url);
        ...
        try {
            ...
            urlParameters.append("room_id=").append(roomID);
            urlParameters.append("&from=").append(from);
            urlParameters.append("&color=").append(color);
            urlParameters.append("&message=").append(message);
            String payload = urlParameters.toString();
            DataOutputStream outputStream  = new DataOutputStream(conn.getOutputStream());
            outputStream.writeBytes(payload);
            outputStream.flush();
            outputStream.close();
